...
const PhotoLightbox = ({ photos, initialIndex, onClose, albumTitle }: PhotoLightboxProps) => {
  ...
  const zoomOut = useCallback(() => setZoom(prev => Math.max(prev - 0.5, 1)), []);
  ...
  const handleShare = useCallback(async () => {
    try {
      if (navigator.share) {
        ...
        toast({ title: "Shared successfully!", description: "Photo shared via native share" });
      } else {
        await navigator.clipboard.writeText(currentPhoto.photo_url);
        toast({ title: "Link copied!", description: "Photo URL copied to clipboard" });
      }
    } catch (error) {
      if (error instanceof Error && error.name !== 'AbortError') {
        try {
          ...
          toast({ title: "Share failed", description: "Unable to share photo", variant: "destructive" });
        }
      }
    }
  }, [currentPhoto, albumTitle, toast]);
  ...
  const handleTouchEnd = () => {
    ...
    if (Math.abs(distanceX) > minSwipeDistance && Math.abs(distanceX) > distanceY) {
      distanceX > 0 ? goToNext() : goToPrevious();
    }
    setTouchStart(null);
    setTouchEnd(null);
  ...
  useEffect(() => {
    if (currentIndex < totalPhotos - 1) {
      const img = new Image();
      img.src = photos[currentIndex + 1].photo_url;
    }
    if (currentIndex > 0) {
      const img = new Image();
      img.src = photos[currentIndex - 1].photo_url;
    ...
    <div ref={containerRef} className="fixed inset-0 z-[100] bg-black flex items-center justify-center">
      ...
      <div className="absolute top-0 left-0 right-0 z-10 bg-gradient-to-b from-black/80 to-transparent p-4 flex items-center justify-between">
        <div className="text-white">
          ...
          <p className="text-sm font-medium">{currentIndex + 1} / {totalPhotos}</p>
        </div>
        <div className="flex gap-2">
          <Button variant="ghost" size="icon" onClick={(e) => { e.stopPropagation(); zoomOut(); }} disabled={zoom <= 1} className="text-white hover:bg-white/20 disabled:opacity-50">
            <ZoomOut className="h-5 w-5" />
          </Button>
          <Button variant="ghost" size="icon" onClick={(e) => { e.stopPropagation(); resetZoom(); }} className="text-white hover:bg-white/20 text-xs px-3">
            {Math.round(zoom * 100)}%
          </Button>
          <Button variant="ghost" size="icon" onClick={(e) => { e.stopPropagation(); zoomIn(); }} disabled={zoom >= 3} className="text-white hover:bg-white/20 disabled:opacity-50">
            <ZoomIn className="h-5 w-5" />
          </Button>
          <Button variant="ghost" size="icon" onClick={(e) => { e.stopPropagation(); handleShare(); }} className="text-white hover:bg-white/20">
      ...
      <Button variant="ghost" size="icon" onClick={(e) => { e.stopPropagation(); goToPrevious(); }} className="absolute left-4 top-1/2 -translate-y-1/2 z-10 h-12 w-12 rounded-full bg-black/50 hover:bg-black/70 text-white">
      ...
      <Button variant="ghost" size="icon" onClick={(e) => { e.stopPropagation(); goToNext(); }} className="absolute right-4 top-1/2 -translate-y-1/2 z-10 h-12 w-12 rounded-full bg-black/50 hover:bg-black/70 text-white">
